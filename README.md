...
> Official C# SDK and WinForms example for [Onyx Gate Auth](https://auth.script-kittens.com) - the authentication platform built for cheat developers.
...
| File | Purpose |
|---|---|
| `SKAuth.cs` | Core SDK - drop into **any** C# project |
...
| `OnyxGate-CSharp.sln` | Visual Studio solution - open this |
...
- Visual Studio 2022
- .NET 8 SDK - [download free at dot.net](https://dotnet.microsoft.com/download)
- NuGet: `System.Management` - **auto-restored on build, no manual install needed**
...
**1. Open the solution**
```
OnyxGate-CSharp.sln → Visual Studio 2022
```

**2. Set your App ID** - open `LoginForm.cs` and change line 11:
```csharp
...
if (result.GetProperty("ok").GetBoolean()) {
    // auth.GetUsername() - logged in user
    // auth.GetPlan()     - "free" / "paid" / "vip" / "lifetime"
    // auth.IsPaid()      - true if paid plan
}
